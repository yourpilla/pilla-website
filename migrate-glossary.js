...
function extractSchemaData(schemaHtml) {
  ...
  if (jsonMatch && jsonMatch[1]) {
    try {
      // Parse and reformat the JSON with proper indentation for YAML literal block
      const jsonData = JSON.parse(jsonMatch[1].trim());
      const jsonString = JSON.stringify(jsonData, null, 2);
      
      // Properly indent each line for YAML literal block (| format)
...
jsonData.forEach((entry, index) => {
  try {
    ...
    // Create YAML frontmatter with proper JSON schema
    const frontmatter = `---
