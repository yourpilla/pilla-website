...
jsonData.forEach((entry, index) => {
  try {
    ...
    // Extract and format schema data - store as simple text to avoid YAML issues
    let schemaData = '';
    if (entry.schema) {
      const jsonMatch = entry.schema.match(/<script[^>]*>([\s\S]*?)<\/script>/);
      if (jsonMatch && jsonMatch[1]) {
        try {
          const parsedSchema = JSON.parse(jsonMatch[1].trim());
          schemaData = JSON.stringify(parsedSchema, null, 2);
        } catch (e) {
          console.warn(`Schema parsing failed for ${slug}`);
        }
      }
    }
    
    // Create YAML frontmatter with schema as quoted string to avoid parsing issues
    const frontmatter = `---
...
if (successCount > 0) {
  ...
  console.log('2. Review a few files to verify format');
